// Creation d'un formulaire directement dans le DOM
...
function accueil2(util){
	...
	if(util.emprunts.length == 0){
		...
		for (emprunt of util.emprunts){
			let iemprunt = document.createElement("LI");
			iemprunt.textContent = emprunt.substrat.titre + " pour le " + emprunt.deadline;
			let brendre =  document.createElement("BUTTON");
			brendre.textContent = "Rendre";
			brendre.addEventListener("click", event => rendre(emprunt, util));
			iemprunt.appendChild(brendre);
			emprunts.appendChild(iemprunt);}}
...
function emprunter(livre, util){
	let url = "emprunt?id="+livre.id+"&login="+util.login;
	let headers = new Headers();
	headers.set('Accept', 'application/json');
...
function rendre(emprunt, util){
	let url = "emprunt?id="+emprunt.id;
	let headers = new Headers();
	headers.set('Accept', 'application/json');
	fetch(url, {method : "delete", headers, mode : "same-origin"})
	.then(function(response){
		accueil(util.login);
	})
	.catch(function(error){
